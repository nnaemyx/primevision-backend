 ...
 *
 * ── Production ──────────────────────────────────────────────────────────
 * Set FROM_EMAIL in your .env to your verified Resend domain address,
 * e.g. "CrestlineTrades <noreply@crestlinetrades.com>"
 *
...
const FROM_EMAIL =
  ...
  (process.env.NODE_ENV === 'production'
    ? 'CrestlineTrades <noreply@crestlinetrades.com>'
    : 'CrestlineTrades <onboarding@resend.dev>');

/** Silent logger — logs email errors without crashing the request */
...
export const sendWelcomeEmail = async (name: string, email: string): Promise<void> => {
  try {
    await resend.emails.send({
      from: FROM_EMAIL,
      to: email,
      subject: 'Welcome to CrestlineTrades!',
      html: `
        <div style="font-family:Arial,sans-serif;background:#0e0e52;color:#fff;padding:40px;border-radius:20px;max-width:600px;margin:auto">
          ...
          <p style="color:#cdcacc;font-size:16px">Thank you for joining CrestlineTrades. Your account has been created and you're ready to start trading.</p>
          ...
          <p style="color:#cdcacc;font-size:12px">CrestlineTrades &copy; 2022-2026, All rights reserved</p>
        </div>
      `,
    });
  } catch (err) {
    logEmailError('sendWelcomeEmail', err);
  }
};
...
export const sendOTPEmail = async (name: string, email: string, otp: string): Promise<void> => {
  try {
    await resend.emails.send({
      from: FROM_EMAIL,
      to: email,
      subject: 'Your CrestlineTrades Verification Code',
      html: `
        <div style="font-family:Arial,sans-serif;background:#0e0e52;color:#fff;padding:40px;border-radius:20px;max-width:600px;margin:auto">
    ...
    logEmailError('sendOTPEmail', err);
...
export const sendPasswordResetEmail = async (
  ...
  try {
    await resend.emails.send({
      from: FROM_EMAIL,
      to: email,
      subject: 'Reset Your CrestlineTrades Password',
      html: `
        <div style="font-family:Arial,sans-serif;background:#0e0e52;color:#fff;padding:40px;border-radius:20px;max-width:600px;margin:auto">
    ...
    logEmailError('sendPasswordResetEmail', err);
...
export const sendDepositConfirmationEmail = async (
  ...
  try {
    await resend.emails.send({
      from: FROM_EMAIL,
      to: email,
      subject: 'Deposit Received – CrestlineTrades',
      html: `
        <div style="font-family:Arial,sans-serif;background:#0e0e52;color:#fff;padding:40px;border-radius:20px;max-width:600px;margin:auto">
    ...
    logEmailError('sendDepositConfirmationEmail', err);
  }
};
...
export const sendWithdrawalEmail = async (
  ...
  try {
    await resend.emails.send({
      from: FROM_EMAIL,
      to: email,
      subject: `Withdrawal ${isApproved ? 'Approved' : 'Rejected'} – CrestlineTrades`,
      html: `
        <div style="font-family:Arial,sans-serif;background:#0e0e52;color:#fff;padding:40px;border-radius:20px;max-width:600px;margin:auto">
    ...
    logEmailError('sendWithdrawalEmail', err);
  }
};
